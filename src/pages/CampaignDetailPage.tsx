  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6 font-sans">
      ...
      <div className="flex items-center mb-8">
        <button onClick={() => navigate('/campaigns')} className="mr-4 text-gray-400 hover:text-white flex items-center">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Campaign: {campaign.name}</h1>
        <div className="flex gap-3">
          <button
            onClick={() => document.getElementById('csv-file-input')?.click()}
            className="px-4 py-2 border border-gray-600 rounded hover:bg-gray-800 flex items-center"
          >
          ...
          {campaign.status === 'running' ? (
            <button
              ...
              className="px-4 py-2 rounded font-medium flex items-center bg-red-600 hover:bg-red-700 text-white"
              title="Stop campaign"
            >
            ...
            <button
              ...
              className="px-4 py-2 rounded font-medium flex items-center bg-blue-600 hover:bg-blue-700 text-white"
            ...
            <button
              ...
              className={`px-4 py-2 rounded font-medium flex items-center ${campaign.status === 'completed'
          ...
          <button
            ...
            className="px-4 py-2 bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white border border-red-600/30 rounded font-medium flex items-center transition"
      ...
      {/* Caller Card */}
      <div className="bg-[#111827] rounded-lg p-6 mb-6 border border-gray-800">
        <div className="flex justify-between items-center">
          <div>
            <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">CALLER</div>
            <div className="flex items-center gap-4">
              {displayPhone !== 'Not Set' ? (
                <div className="flex flex-col">
                  <span className="text-xl font-mono">{displayPhone}</span>
                  <span className="text-sm text-gray-400">Agent: {displayAgent}</span>
                </div>
              ) : (
                <button
                  onClick={() => setIsCallerPhoneModalOpen(true)}
                  className="px-4 py-2 bg-gray-800 border border-emerald-500/50 text-emerald-500 rounded hover:bg-gray-700"
                >
                  Set Caller Phone
                </button>
              )}
          ...
          <div>
            ...
            <span className={`px-3 py-1 rounded text-sm font-medium border ${campaign.status === 'running' ? 'bg-green-900/30 text-green-400 border-green-900' :
              campaign.status === 'completed' ? 'bg-blue-900/30 text-blue-400 border-blue-900' :
      ...
      <div className="flex items-center mb-8">
        <button
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${includeMetadata ? 'bg-emerald-500' : 'bg-gray-700'}`}
        ...
        <span className="ml-3 text-sm text-gray-300">Include extra metadata in agent prompt <a href="#" className="text-blue-400 hover:underline">Learn more</a></span>
      </div>

      {/* Records Table */}
      <div className="bg-[#111827] rounded-lg border border-gray-800 overflow-hidden">
        <div className="grid grid-cols-[1fr_1fr_auto] p-4 border-b border-gray-800 bg-[#1F2937]">
          <div className="text-sm font-medium text-gray-400">Phone</div>
          <div className="text-sm font-medium text-gray-400">Call Status</div>
          <div className="text-sm font-medium text-gray-400 text-right">Actions</div>
        </div>

        {records.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-gray-500 mb-4">No campaign record found</p>
            <button
              onClick={() => setIsAddRecordModalOpen(true)}
              className="px-4 py-2 border border-emerald-500/50 text-emerald-500 rounded hover:bg-gray-800"
            >
              + Add Record
            </button>
          </div>
          ...
          <div className="divide-y divide-gray-800">
            {records.map((record) => (
              <div key={record.id} className="grid grid-cols-[1fr_1fr_auto] p-4 items-center hover:bg-gray-800/50">
                <div className="font-mono text-gray-300">{record.phone}</div>
                <div>
                  <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${record.callStatus === 'completed' ? 'bg-green-900/30 text-green-400 border border-green-900' :
                    record.callStatus === 'calling' ? 'bg-blue-900/30 text-blue-400 border border-blue-900' :
                      record.callStatus === 'failed' ? 'bg-red-900/30 text-red-400 border border-red-900' :
                        'bg-gray-800 text-gray-400 border border-gray-700'
                    }`}>
                    {record.callStatus || 'Pending'}
                  </span>
                  {record.callStatus === 'completed' && record.recordingUrl && (
                    <a href={record.recordingUrl} target="_blank" className="ml-3 text-xs text-blue-400 hover:underline">Recording</a>
                  )}
                </div>
      ...
      {isCallerPhoneModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#1F2937] p-6 rounded-lg w-full max-w-md">
            <h3 className="text-xl font-bold mb-4">Set Caller Phone</h3>
            <div className="mb-4">
              <label className="block text-sm text-gray-400 mb-2">Select Number</label>
              <select
                value={selectedPhoneNumberId}
                onChange={(e) => setSelectedPhoneNumberId(e.target.value)}
                className="w-full bg-[#374151] border border-gray-600 rounded p-2 text-white"
              >
              ...
              <p className="text-xs text-gray-500 mt-2">Only showing numbers with assigned agents.</p>
            </div>
            <div className="flex justify-end gap-3">
              <button onClick={() => setIsCallerPhoneModalOpen(false)} className="px-4 py-2 text-gray-300 hover:text-white">Cancel</button>
              <button onClick={handleSetCallerPhone} className="px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isAddRecordModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#1F2937] p-6 rounded-lg w-full max-w-md">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setIsAddRecordModalOpen(false)} className="px-4 py-2 text-gray-300 hover:text-white">Cancel</button>
              <button onClick={handleAddRecord} className="px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700">Add</button>
            </div>
          </div>
        </div>
      )}
